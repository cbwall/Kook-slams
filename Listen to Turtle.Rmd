...
# make sure important columns are factors
run.metaD$year<-as.factor(run.metaD$year)
run.metaD$site<-as.factor(run.metaD$site)
run.metaD$sample_control<-as.factor(run.metaD$sample_control)
```
...
If running SILVA assignment is problemtic, skip this step and jumpt to the one below where you can load in the taxanomic assignment as a .RData file `taxaTable.rds`. 
...
```{r load in the taxaTable Rdata}
# this will load in the data I generated by running the code chunk above.
taxa<- readRDS("output/taxTab.rds")
```

Alternative to using `assignTaxonomy` can be using DECIPHER to load in data, however the versions are different (2019-v138 vs. 2021-v138.1)
```{r Decipher, eval=FALSE}
# this is an example from the DADA2 page on how to use DECIPHER
# it has not been edited (yet) to work with the data in this tutorial
...
phylo.rich.plot<-plot_richness(ps, x="site", measures=c("Observed", "Shannon"))
